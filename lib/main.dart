...
class MapRequestItem {
  ...
  final String size;
  final bool isFrozen;
  final double mapSeed; // Used to randomize map backgrounds/pin locations
...
// 4 Ongoing/Active Deliveries within constraints
const List<DeliveryItem> mockOngoingDeliveries = [
  DeliveryItem(store: 'E.Leclerc Drive', location: 'Melun, Île-de-France', amount: 6.20, dateStr: '2 days ago', size: 'M', isFrozen: false),
  DeliveryItem(store: 'Carrefour Market', location: 'Versailles, Île-de-France', amount: 5.80, dateStr: '3 days ago', size: 'L', isFrozen: true),
  DeliveryItem(store: 'Auchan Supermarché', location: 'Évry, Île-de-France', amount: 6.45, dateStr: '1 day ago', size: 'S', isFrozen: false),
  DeliveryItem(store: 'Intermarché', location: 'Paris 15e, Île-de-France', amount: 4.90, dateStr: '4 days ago', size: 'M', isFrozen: true),
];

// Dynamic map request mock data generated for 20 items
final List<MapRequestItem> mockMapRequests = List.generate(20, (index) {
  ...
  final amount = 4.50 + rand.nextDouble() * 1.99; // €4.50 to €6.49
...
class _ShopopopCloneState extends State<ShopopopClone> {
  ...
  @override
  Widget build(BuildContext context) {
    // Dynamically expand to match the full height/proportions of Samsung A20s aspect ratio
    return Scaffold(
...
// ==========================================
// SEARCH SCREEN (DYNAMIC SWIPEABLE MAP CAROUSEL)
// ==========================================
...
class _DynamicMapArea extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Generate pseudorandom shifting layouts based on item seed value
    final double offsetA = (seed * 1.3) % 80;
    final double offsetB = (seed * 2.7) % 100;
    ...
    return Container(
      ...
      child: Stack(
        children: [
          // Simulated grid streets that alter backgrounds dynamically per page index
          Positioned(
            top: 100 + (index * 12) % 150,
            left: -50, right: -50,
            child: Transform.rotate(
              angle: (index.isEven ? -4 : 4) * math.pi / 180,
              child: Container(height: 4, color: const Color(0xFFF3D9A0)),
            ),
          ),
          Positioned(
            top: 250 - (index * 8) % 120,
            left: -50, right: -50,
            child: Transform.rotate(
              angle: (index.isOdd ? -2 : 6) * math.pi / 180,
              child: Container(height: 4, color: const Color(0xFFF3D9A0)),
            ),
          ),
          Positioned(
            left: 120 + (index * 15) % 160,
            top: -50, bottom: -50,
            child: Transform.rotate(
              angle: 5 * math.pi / 180,
              child: Container(width: 4, color: const Color(0xFFF6E7C4)),
            ),
          ),
          // Shifting Pickup Pin A
          Positioned(
            top: 80 + offsetA,
            left: 70 + offsetB,
            child: const _PinBadge('A'),
          ),
          // Shifting Dropoff Pin B
          Positioned(
            bottom: 90 + offsetB,
            right: 60 + offsetA,
          ...
          Positioned(
            ...
            child: Center(
              child: Container(
                ...
                child: Text(
                  '${(3.1 + (index * 0.4) % 8).toStringAsFixed(1)} km - ${10 + index % 15} min',
...
// ==========================================
// DELIVERIES SCREEN (EXPANDED DYNAMIC BALANCED ENTRIES)
// ==========================================
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Text('Nicolau', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: AppColors.navy)),
          ),
          Padding(
            padding: const EdgeInsets.all(20),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('My account', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.navy, fontSize: 15)),
                const SizedBox(height: 10),
                Row(
                  children: [
                    Expanded(
                    ...
                    Expanded(
                      child: Container(
                        ...
                        child: const Text('Regular trips', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                      ),
                    ),
                  ],
                ),
                const SizedBox(height: 20),
                _AccountRow(label: 'My personal information'),
                _AccountRow(label: 'Notifications and alerts'),
                _AccountRow(label: 'Log out', isDestructive: true),
              ],
            ),
          ),
        ],
...
class _AccountRow extends StatelessWidget {
  ...
  const _AccountRow({required this.label, this.isDestructive = false});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text(label, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: isDestructive ? AppColors.coralDeep : AppColors.navy)),
          Icon(isDestructive ? LucideIcons.power : LucideIcons.arrowRight, size: 16, color: isDestructive ? AppColors.coralDeep : AppColors.blue),
        ],
      ),
    );
  }
