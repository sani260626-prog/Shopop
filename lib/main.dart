...
// Completed Deliveries modified to include item entries today adjusting to exactly €178.44
const List<DeliveryItem> mockCompletedDeliveries = [
  DeliveryItem(store: 'Monoprix', location: 'Paris 11e, Île-de-France', amount: 8.50, dateStr: 'Today', size: 'M', isFrozen: false),
  DeliveryItem(store: 'E.Leclerc Drive', location: 'Asnières, Île-de-France', amount: 7.20, dateStr: 'Today', size: 'L', isFrozen: false),
  DeliveryItem(store: 'Auchan Supermarché', location: 'Massy, Île-de-France', amount: 6.90, dateStr: 'Today', size: 'S', isFrozen: true),
  DeliveryItem(store: 'Carrefour Market', location: 'Nanterre, Île-de-France', amount: 10.19, dateStr: 'Today', size: 'XL', isFrozen: false),
...
// Dynamic map requests generated for 20 items (Updated times and adapted locations)
final List<MapRequestItem> mockMapRequests = List.generate(20, (index) {
  ...
  final towns = ['Massy', 'Boulogne-Billancourt', 'Saint-Cloud', 'Neuilly-sur-Seine', 'Sartrouville', 'Asnières', 'Meaux', 'Rambouillet'];
  ...
  final amount = 5.20 + rand.nextDouble() * 3.50;
  
  // Custom modified hour blocks to update the operational time frames
  final startHour = 10 + (index % 7);
  final endHour = startHour + 1 + (index % 2);
  
  return MapRequestItem(
    store: store,
    route: '$store - $town, Île-de-France',
    amount: double.parse(amount.toStringAsFixed(2)),
    timeWindow: 'Pick up : ${startHour}h${10 + (index * 2) % 45} - ${endHour}h${10 + (index * 2) % 45}',
    size: size,
    isFrozen: isFrozen,
    mapSeed: index * 45.0,
  );
...
class _DynamicMapArea extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Modified dynamic pinning layouts to vary distances and position elements further apart
    final double offsetA = 20.0 + (math.sin(seed) * 90.0).abs() + (seed % 100);
    final double offsetB = 10.0 + (math.cos(seed) * 110.0).abs() + (seed % 120);
          ...
          Positioned(
            ...
            child: Center(
              child: Container(
                ...
                child: Text(
                  '${(4.8 + (index * 1.4) % 15).toStringAsFixed(1)} km - ${12 + index % 28} min',
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: const Text('€178.44', style: TextStyle(color: Colors.white, fontSize: 32, fontWeight: FontWeight.bold)),
          ),
          const SizedBox(height: 20),
          Row(
            children: const [
